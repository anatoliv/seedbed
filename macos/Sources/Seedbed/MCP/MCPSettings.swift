...
struct MCPSettings: View {
    ...
    private var header: some View {
        HStack {
            Text("MCP Server").font(.system(size: Tokens.CompactSize.heading, weight: .semibold))
            Spacer()
        ...
        HStack(alignment: .top, spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                Text("Let an agent ask for a prompt")
                    .font(.system(size: Tokens.CompactSize.rowText, weight: .medium))
    ...
    private var serverSection: some View {
        section("Server") {
            ...
            HStack(spacing: 8) {
                Text("Port").font(.system(size: Tokens.CompactSize.rowText))
    ...
    private var statusRow: some View {
        HStack(spacing: 6) {
            ...
            if server.isRunning {
                Text("Running at \(url)").font(.system(size: Tokens.CompactSize.meta))
            } else if let error = server.lastError {
                Text(error).font(.system(size: Tokens.CompactSize.meta)).foregroundStyle(.red)
            } else {
                Text("Not running").font(.system(size: Tokens.CompactSize.meta))
    ...
    private func note(_ text: String) -> some View {
        Text(text).font(.system(size: Tokens.CompactSize.meta)).foregroundStyle(.secondary)
            .fixedSize(horizontal: false, vertical: true)
    }
